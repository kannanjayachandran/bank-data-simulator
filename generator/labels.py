...
def generate_feature_snapshots(
    ...
    eligible_labels = customer_churn_label.filter(
        pl.col("as_of_month").is_in(months_set)
    )
    ...
    base_df = base_df.with_columns(
        (
            ...
            + (pl.col("as_of_month").dt.month() - pl.col("customer_since").dt.month())
        )
        .cast(pl.Int32)
        .alias("tenure_months")
    )
    ...
    if "customer_id" not in account_monthly_snapshot.columns:
        account_snapshots = account_monthly_snapshot.join(
            account_master.select(["account_id", "customer_id"]),
            on="account_id",
            how="left",
        )
    else:
    ...
    if "customer_id" not in card_monthly_snapshot.columns:
        card_snapshots = card_monthly_snapshot.join(
            card_portfolio.select(["card_id", "customer_id"]), on="card_id", how="left"
        )
    else:
        card_snapshots = card_monthly_snapshot

    if "customer_id" not in loan_monthly_snapshot.columns:
        loan_snapshots = loan_monthly_snapshot.join(
            loan_master.select(["loan_id", "customer_id"]), on="loan_id", how="left"
        )
    else:
    ...
    base_df = base_df.with_columns(
        [
            pl.coalesce(pl.col("prod_cnt_m1"), 1)
            .cast(pl.Int32)
            .alias("products_count"),
            pl.max_horizontal(
                [
                    0,
                    pl.coalesce(pl.col("prod_cnt_m1"), 0)
                    - pl.coalesce(pl.col("prod_cnt_m6"), 0),
                ]
            )
            .cast(pl.Int32)
            .alias("product_acquisition_velocity_6m"),
        ]
    )

    # 3. balance_change_3m
    cust_monthly_bal = account_snapshots.group_by(
        ["customer_id", "snapshot_month"]
    ).agg(pl.col("current_balance").mean().alias("avg_bal"))
    for i in range(1, 7):
    ...
    base_df = base_df.with_columns(
        [
            pl.coalesce(pl.mean_horizontal(["bal_m1", "bal_m2", "bal_m3"]), 0.0).alias(
                "avg_bal_recent"
            ),
            pl.coalesce(pl.mean_horizontal(["bal_m4", "bal_m5", "bal_m6"]), 0.0).alias(
                "avg_bal_prior"
            ),
        ]
    )
    base_df = base_df.with_columns(
        ...
        .then(
            (pl.col("avg_bal_recent") - pl.col("avg_bal_prior"))
            / pl.col("avg_bal_prior")
        )
        .otherwise(0.0)
        .round(4)
    ...
    base_df = base_df.with_columns(
        [
            pl.coalesce(
                pl.mean_horizontal(["txns_m1", "txns_m2", "txns_m3"]), 0.0
            ).alias("avg_txn_recent"),
            pl.coalesce(
                pl.mean_horizontal(["txns_m4", "txns_m5", "txns_m6"]), 0.0
            ).alias("avg_txn_prior"),
            pl.coalesce(
                pl.mean_horizontal([f"logins_m{i}" for i in range(1, 7)]), 0.0
            ).alias("avg_login_recent"),
            pl.coalesce(
                pl.mean_horizontal([f"logins_m{i}" for i in range(7, 13)]), 0.0
            ).alias("avg_login_prior"),
        ]
    )
    base_df = base_df.with_columns(
        [
            ...
            .then(
                (pl.col("avg_txn_recent") - pl.col("avg_txn_prior"))
                / pl.col("avg_txn_prior")
            )
            .otherwise(0.0)
            .round(4)
            ...
            .then(
                (pl.col("avg_login_recent") - pl.col("avg_login_prior"))
                / pl.col("avg_login_prior")
            )
            .otherwise(0.0)
            .round(4)
            .alias("login_count_change_6m"),
            pl.coalesce(pl.col("dorm_days_m1"), 180)
            .cast(pl.Int32)
            .alias("dormant_days"),
        ]
    )
    ...
    comp_6m_df = (
        cust_months.join(customer_complaints, on="customer_id", how="inner")
        .filter(
            (pl.col("complaint_month") >= pl.col("as_of_month").dt.offset_by("-6mo"))
            & (pl.col("complaint_month") < pl.col("as_of_month"))
        )
        .group_by(["customer_id", "as_of_month"])
        .len()
        .rename({"len": "comp_count_6m"})
    )

    unres_df = (
        cust_months.join(
            customer_complaints.filter(~pl.col("resolved_flag")),
            on="customer_id",
            how="inner",
        )
        .filter(pl.col("complaint_month") < pl.col("as_of_month"))
        .group_by(["customer_id", "as_of_month"])
        .len()
        .rename({"len": "unres_count"})
    )
    ...
    base_df = base_df.with_columns(
        [
            pl.coalesce(pl.col("comp_count_6m"), 0)
            .cast(pl.Int32)
            .alias("complaint_count_6m"),
            pl.coalesce(pl.col("unres_count"), 0)
            .cast(pl.Int32)
    ...
    base_df = base_df.with_columns(
        pl.coalesce(
            (pl.col("as_of_month") - pl.col("last_login_date")).dt.total_days(), 180
        )
        .cast(pl.Int32)
    ...
    sal_months = (
        account_snapshots.filter(pl.col("salary_credit_amount") > 0)
        .select(["customer_id", "snapshot_month"])
        .unique()
    )
    sal_consistency_df = (
        cust_months.join(sal_months, on="customer_id", how="inner")
        .filter(
    ...
    base_df = base_df.with_columns(pl.col("salary_credit_consistency").fill_null(0.0))
    ...
    base_df = base_df.with_columns(pl.col("credit_utilization").fill_null(0.0).round(4))
    ...
    nps_df = (
        ...
        .group_by(["customer_id", "as_of_month"])
        .agg(pl.col("nps_score").mean().alias("avg_nps"))
    )
    ...
    campaign_df = (
        cust_months.join(digital_engagement_monthly, on="customer_id", how="inner")
        .filter(
        ...
        .select(
            [
                "customer_id",
                "as_of_month",
                pl.when(pl.col("recv") > 0)
                .then(pl.col("resp") / pl.col("recv"))
                .otherwise(0.0)
                .alias("campaign_response_rate"),
            ]
        )
    )
