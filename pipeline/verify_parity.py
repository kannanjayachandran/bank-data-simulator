...
def check_fk_integrity(data_dir: str) -> bool:
    ...
    try:
        ...
        def assert_fk(child_df, child_col, parent_set, name):
            ...
            if invalid:
                print(
                    f"  ❌ FK Violation: {name}.{child_col} has {len(invalid)} values not in parent! (e.g. {invalid[:5]})"
                )
        ...
        acc_snap = pl.read_parquet(
            os.path.join(data_dir, "account_monthly_snapshot/**/*.parquet")
        )
        ...
        card_snap = pl.read_parquet(
            os.path.join(data_dir, "card_monthly_snapshot/**/*.parquet")
        )
        ...
        loan_snap = pl.read_parquet(
            os.path.join(data_dir, "loan_monthly_snapshot/**/*.parquet")
        )
        assert_fk(loan_snap, "loan_id", loan_ids, "loan_monthly_snapshot")

        # product_holdings_monthly
        holdings = pl.read_parquet(
            os.path.join(data_dir, "product_holdings_monthly/**/*.parquet")
        )
        ...
        activity = pl.read_parquet(
            os.path.join(data_dir, "customer_monthly_activity/**/*.parquet")
        )
        assert_fk(activity, "customer_id", cust_ids, "customer_monthly_activity")

        # digital_engagement_monthly
        digital = pl.read_parquet(
            os.path.join(data_dir, "digital_engagement_monthly/**/*.parquet")
        )
        assert_fk(digital, "customer_id", cust_ids, "digital_engagement_monthly")

        # customer_complaints
        comp = pl.read_parquet(
            os.path.join(data_dir, "customer_complaints/**/*.parquet")
        )
...
def main():
    ...
    if not os.path.exists(optimized_dir):
        print(
            f"Error: Optimized directory '{optimized_dir}' does not exist. Run the optimized simulation first."
        )
        sys.exit(1)
    ...
    sim_base = pl.read_parquet(
        os.path.join(baseline_dir, "churn_simulation_state.parquet")
    )
    sim_opt = pl.read_parquet(
        os.path.join(optimized_dir, "churn_simulation_state.parquet")
    )

    label_base = pl.read_parquet(
        os.path.join(baseline_dir, "customer_churn_label/**/*.parquet")
    )
    label_opt = pl.read_parquet(
        os.path.join(optimized_dir, "customer_churn_label/**/*.parquet")
    )

    # Join churn_simulation_state to get persona
    label_base = label_base.join(
        sim_base.select(["customer_id", "persona"]), on="customer_id"
    )
    label_opt = label_opt.join(
        sim_opt.select(["customer_id", "persona"]), on="customer_id"
    )
    ...
    personas_base = (
        label_base.group_by("persona")
        .agg(pl.col("churned").mean().alias("churn_rate"))
        .sort("persona")
    )
    personas_opt = (
        label_opt.group_by("persona")
        .agg(pl.col("churned").mean().alias("churn_rate"))
        .sort("persona")
    )
    ...
    for row_b in personas_base.iter_rows(named=True):
        ...
        print(
            f"  - {persona:25}: Baseline={rate_b:.2%}, Optimized={rate_o:.2%}, Diff={diff:.2%} {status}"
        )
    ...
    # Final balances
    acct_base = pl.read_parquet(
        os.path.join(baseline_dir, "account_monthly_snapshot/**/*.parquet")
    )
    acct_opt = pl.read_parquet(
        os.path.join(optimized_dir, "account_monthly_snapshot/**/*.parquet")
    )
    ...
    bal_base = acct_base.filter(pl.col("snapshot_month") == max_month_base)[
        "current_balance"
    ].to_numpy()
    bal_opt = acct_opt.filter(pl.col("snapshot_month") == max_month_opt)[
        "current_balance"
    ].to_numpy()
    ks_balance = ks_statistic(bal_base, bal_opt)

    # Transaction count
    txn_base_df = pl.read_parquet(
        os.path.join(baseline_dir, "transaction_fact/**/*.parquet")
    )
    txn_opt_df = pl.read_parquet(
        os.path.join(optimized_dir, "transaction_fact/**/*.parquet")
    )
    ...
    print(
        f"  - Annual Income Distribution KS Stat:    {ks_income:.4f} (Target <= 0.05)"
    )
    print(
        f"  - Final Account Balance Distribution KS:  {ks_balance:.4f} (Target <= 0.07)"
    )
