...
def test_sql_features_match_python():
    ...
    con.execute(
        "ALTER TABLE churn_feature_snapshot RENAME TO churn_feature_snapshot_python_baseline"
    )

    # Create empty churn_feature_snapshot table (using same schema)
    con.execute(
        "CREATE TABLE churn_feature_snapshot AS SELECT * FROM churn_feature_snapshot_python_baseline WHERE FALSE"
    )
    ...
    assert len(sql_features) == len(py_features), (
        f"Row count mismatch: SQL={len(sql_features)}, Python={len(py_features)}"
    )
    ...
    for col in py_features.columns:
        ...
        # Check type
        assert sql_col.dtype == py_col.dtype, (
            f"Column '{col}' type mismatch: SQL={sql_col.dtype}, Python={py_col.dtype}"
        )
        ...
        if py_col.dtype in (pl.Float64, pl.Float32):
            ...
            for idx, (s_val, p_val) in enumerate(zip(sql_col, py_col)):
                if s_val is None or p_val is None:
                    assert s_val == p_val, (
                        f"Null mismatch in '{col}' at index {idx}: SQL={s_val}, Python={p_val}"
                    )
                else:
                    assert abs(s_val - p_val) < 2e-4, (
                        f"Value mismatch in '{col}' at index {idx}: SQL={s_val}, Python={p_val}"
                    )
        else:
    ...
    print(
        "✔ SQL features match Python baseline exactly (within float precision tolerances)."
    )
