...
def test_simulation_outputs_exist(sim_results):
    ...
    for key in expected_keys:
        ...
        assert isinstance(results[key], pl.DataFrame), (
            f"Table {key} is not a Polars DataFrame!"
        )
...
def test_no_future_activity_post_churn(sim_results):
    ...
    churned_custs = state_df.filter(pl.col("churned_flag")).to_dicts()
    ...
    for c in churned_custs:
        ...
        cust_acc = acc_snap.filter(
            (
                pl.col("account_id").is_in(
                    results["account_master"]
                    .filter(pl.col("customer_id") == cid)["account_id"]
                    .implode()
                )
            )
        )
        for row in cust_acc.to_dicts():
            assert row["snapshot_month"] <= c_month, (
                f"Account snapshot for customer {cid} found in month {row['snapshot_month']} after churn month {c_month}!"
            )

        cust_act = act_snap.filter(pl.col("customer_id") == cid)
        for row in cust_act.to_dicts():
            assert row["snapshot_month"] <= c_month, (
                f"Activity snapshot for customer {cid} found in month {row['snapshot_month']} after churn month {c_month}!"
            )

        cust_txns = txn_fact.filter(pl.col("customer_id") == cid)
        for row in cust_txns.to_dicts():
            assert row["txn_month"] <= c_month, (
                f"Transaction for customer {cid} found in month {row['txn_month']} after churn month {c_month}!"
            )
...
def test_behavioral_validations(sim_results):
    ...
    sc_churn_rate = state_df.filter(pl.col("persona") == Persona.SALARY_CORE.value)[
        "churned_flag"
    ].mean()
    ...
    ).join(state_df.select(["customer_id", "persona"]), on="customer_id")

    cs_avg_dpd = ln_merged.filter(pl.col("persona") == Persona.CREDIT_STRESSED.value)[
        "dpd_days"
    ].mean()
...
def test_sequential_identical_regression():
    ...
    for key in res_sequential.keys():
        ...
        assert df_seq.equals(df_def), (
            f"Mismatch in table {key} between jobs=1 and default run!"
        )
...
def test_parallel_simulation_equivalence():
    ...
    for key in res_seq.keys():
        ...
        assert res_par[key].schema == res_seq[key].schema, (
            f"Schema mismatch in table {key}!"
        )
    ...
    if not res_par["transaction_fact"].is_empty():
        ...
        assert len(txn_ids) == len(set(txn_ids)), (
            "Duplicate transaction_ids found in transaction_fact!"
        )
    ...
    if not res_par["customer_complaints"].is_empty():
        ...
        assert len(comp_ids) == len(set(comp_ids)), (
            "Duplicate complaint_ids found in customer_complaints!"
        )
    ...
    if not res_par["customer_feedback"].is_empty():
        ...
        assert len(feed_ids) == len(set(feed_ids)), (
            "Duplicate feedback_ids found in customer_feedback!"
        )
    ...
    assert_fk_exists(
        res_par["account_master"], "customer_id", cust_ids_set, "account_master"
    )
    assert_fk_exists(
        res_par["account_monthly_snapshot"],
        "account_id",
        acc_ids_set,
        "account_monthly_snapshot",
    )
    assert_fk_exists(
        res_par["card_portfolio"], "customer_id", cust_ids_set, "card_portfolio"
    )
    assert_fk_exists(
        res_par["card_monthly_snapshot"],
        "card_id",
        card_ids_set,
        "card_monthly_snapshot",
    )
    ...
    assert_fk_exists(
        res_par["loan_monthly_snapshot"],
        "loan_id",
        loan_ids_set,
        "loan_monthly_snapshot",
    )
    assert_fk_exists(
        res_par["product_holdings_monthly"],
        "customer_id",
        cust_ids_set,
        "product_holdings_monthly",
    )
    assert_fk_exists(
        res_par["transaction_fact"], "customer_id", cust_ids_set, "transaction_fact"
    )
    assert_fk_exists(
        res_par["transaction_fact"], "account_id", acc_ids_set, "transaction_fact"
    )
    assert_fk_exists(
        res_par["customer_monthly_activity"],
        "customer_id",
        cust_ids_set,
        "customer_monthly_activity",
    )
    assert_fk_exists(
        res_par["digital_engagement_monthly"],
        "customer_id",
        cust_ids_set,
        "digital_engagement_monthly",
    )
    assert_fk_exists(
        res_par["customer_complaints"],
        "customer_id",
        cust_ids_set,
        "customer_complaints",
    )
    assert_fk_exists(
        res_par["customer_feedback"], "customer_id", cust_ids_set, "customer_feedback"
    )
    assert_fk_exists(
        res_par["churn_simulation_state"],
        "customer_id",
        cust_ids_set,
        "churn_simulation_state",
    )
    assert_fk_exists(
        res_par["customer_churn_label"],
        "customer_id",
        cust_ids_set,
        "customer_churn_label",
    )
    assert_fk_exists(
        res_par["churn_feature_snapshot"],
        "customer_id",
        cust_ids_set,
        "churn_feature_snapshot",
    )
    ...
    labels_seq = res_seq["customer_churn_label"].join(
        res_seq["churn_simulation_state"].select(["customer_id", "persona"]),
        on="customer_id",
    )
    labels_par = res_par["customer_churn_label"].join(
        res_par["churn_simulation_state"].select(["customer_id", "persona"]),
        on="customer_id",
    )

    seq_persona_churn = labels_seq.group_by("persona").agg(
        pl.col("churned").mean().alias("churn_rate")
    )
    par_persona_churn = labels_par.group_by("persona").agg(
        pl.col("churned").mean().alias("churn_rate")
    )
    ...
    for row in seq_persona_churn.iter_rows(named=True):
        ...
        if not par_row.is_empty():
            ...
            assert abs(seq_rate - par_rate) <= 0.10, (
                f"Churn rate deviation too high for {persona}: sequential={seq_rate:.2%}, parallel={par_rate:.2%}"
            )
    ...
    for table in res_seq.keys():
        ...
        if seq_len > 0:
            ...
            assert diff_pct <= 0.25, (
                f"Row count deviation too high for table {table}: seq={seq_len}, par={par_len} ({diff_pct:.1%})"
            )
